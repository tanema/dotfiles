local colors = {
	normal = { fg = '#000000', bg = '#8fbfdc', ctermfg = 'black', ctermbg = 'cyan' },
	insert = { fg = '#000000', bg = '#d7afff', ctermfg = 'black', ctermbg = 'magenta' },
...
_G.modeName = function()
	...
	if startsWith(vmode, 'n') then
		return 'Normal'
	elseif startsWith(vmode, 'v') then
		return 'Visual'
	elseif startsWith(vmode, 'V') then
		return 'V·Line'
	elseif startsWith(vmode, '') then
		return 'V·Block'
	elseif startsWith(vmode, 's') then
		return 'Select'
	elseif startsWith(vmode, 'S') then
		return 'S·Line'
	elseif startsWith(vmode, '') then
		return 'S·Block'
	elseif startsWith(vmode, 'i') then
		return 'Insert'
	elseif startsWith(vmode, 'R') then
		return 'Replace'
	elseif startsWith(vmode, 'c') then
		return 'Command'
	elseif startsWith(vmode, 'r') then
		return 'Confirm'
	elseif startsWith(vmode, 't') then
		return 'Terminal'
	elseif vmode == '!' then
		return 'Shell'
	else
